  ...
  return (
    <>
      <ListItem
        ...
        disablePadding
        secondaryAction={
          _.map(toggleItemActive, (sel) => Number(sel.id)).includes(
            item.id
      ...
      >
        <ListItemButton
          ...
          selected={_.map(toggleItemActive, (sel) => Number(sel.id)).includes(
            item.id
          )}
        >
...
export default function VirtualListSelection({
  ...
  const handleClick = (e, data) => {
    ...
    handleSave({
      is_parent: isParent,
      id: isSubtask == null ? taskId : isSubtask,
      key: type,
      value: data.id,
      selectedArr: {
  ...
  const itemData = createItemData(
    _.filter(
      _.sortBy(option, (type) =>
        _.map(selected, (sel) => Number(sel.id)).includes(type.id) ? 0 : 1
      ),
      ...
      {!_.isEmpty(
        ...
        <FixedSizeList
          ...
          itemCount={
            _.filter(
              _.sortBy(option, (type) =>
                _.map(selected, (sel) => Number(sel.id)).includes(type.id)
                  ? 0
                  : 1
              ),
              (data) =>
