    ...
    <>
      {isEditingThread && thread?.id === selectedThreadId ? (
        ...
        >
          <CommentHeader
            user={{
              name: thread?.created_by?.name,
              avatar: thread?.created_by?.avatar,
              id: thread?.created_by?.user_id,
            }}
            createdDate={dateFormatter(thread?.created_at)}
            commentRelType={commentRelType}
