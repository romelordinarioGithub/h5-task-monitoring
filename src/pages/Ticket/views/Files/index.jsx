...
// Reducer
import { getTicketFiles } from 'store/reducers/ticket';
...
const Files = () => {
  ...
  // Redux State
  const { ticket, files } = useSelector((state) => state.ticket);
  const { handleModal } = useContext(TicketContext);

  useOnMount(() => {
    dispatch(getTicketFiles(ticket.id));
  });
