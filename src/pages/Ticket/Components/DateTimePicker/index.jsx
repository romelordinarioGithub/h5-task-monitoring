...
export default function DateTimerPicker({
  ...
  const handleCheckLimitDate = () => {
    if (
      _.isEqual(type, 'date_started') &&
      new Date(`${date} ${time}`) > new Date(limit)
    )
      return true;
    else if (
      _.isEqual(type, 'date_ended') &&
      new Date(`${date} ${time}`) < new Date(limit)
    )
      return true;
    else return false;
  ...
  const handleOnClickApply = () =>
    handleSave({
      id: taskId,
      key: type,
      value: dayjs(`${date} ${time}`).format('MM/DD/YYYY hh:mm:ss A'),
      limit: dayjs(limit).format('MM/DD/YYYY hh:mm:ss A'),
    });
      ...
      <Stack direction="row">
        <Stack>
          <StaticDatePicker
            ...
            minDate={
              _.isEqual(type, 'date_ended') ? dayjs(limit ?? new Date()) : null
            }
