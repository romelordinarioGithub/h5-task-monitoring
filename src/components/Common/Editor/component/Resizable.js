...
export default class ResizableHeight extends Plugin {
  init() {
    ...
    this.editor.on('ready', () => {
      const editorMainElement = editor.ui.view.element.querySelector(
          '.ck.ck-editor__main'
        ),
        // editorContentElement = editorMainElement.querySelector(
        //   '.ck.ck-content.ck-editor__editable'
        // ),
        height =
          editor.sourceElement.getAttribute('data-height') ||
          editor.config.get('ResizableHeight.height'),
        resize = editor.config.get('ResizableHeight.resize');

      editor.editing.view.document.on('focus', () => {
        const editorMainElement =
          editor.ui.view.element.querySelector('.ck-editor__main');
      ...
      if (resize === undefined || resize === true) {
        const minHeight =
            editor.sourceElement.getAttribute('data-minheight') ||
            editor.config.get('ResizableHeight.minHeight'),
          maxHeight =
            editor.sourceElement.getAttribute('data-maxheight') ||
            editor.config.get('ResizableHeight.maxHeight');
        if (
          minHeight === undefined ||
          maxHeight === undefined ||
          minHeight !== maxHeight
